...
import { ILinkClient, loginWithQR, MessageType, MessageItemType, } from "weixin-ilink";
import fs from "node:fs";
import path from "node:path";
import { loadCredentials, loadSyncBuf, saveSyncBuf, loadContextTokens, getContextToken, setContextToken, loadChannelConfig, saveCredentials, STATE_DIR, } from "./store.js";
...
const log = (...a) => console.error("[weixin-channel]", ...a);
...
async function serverLogin(mcp) {
    ...
    const result = await loginWithQR({
        onQRCode: async (url) => {
            ...
            mcp
                ...
                params: {
                    content: "微信尚未登录。请扫描下方二维码完成登录（在微信中扫码并确认）：\n\n" +
                        ascii +
...
async function main() {
    ...
    const mcp = new Server({ name: "weixin", version: "0.1.0" }, {
        capabilities: {
            experimental: { "claude/channel": {} },
            tools: {},
        },
        instructions: 'Messages from your WeChat arrive as <channel source="weixin" user_id="...">...</channel>. ' +
    ...
    mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: "reply",
                description: "Send a message back to the WeChat user who messaged you. " +
    ...
    mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
        if (req.params.name === "reply") {
            ...
            if (!userId || !text) {
                return {
                    content: [{ type: "text", text: "user_id 和 text 均为必填" }],
                    isError: true,
                };
            }
            if (!client) {
                return {
                    content: [{ type: "text", text: "weixin channel 未连接（缺少登录凭证）" }],
                    isError: true,
                };
            }
            ...
            try {
                ...
                return { content: [{ type: "text", text: `已发送回复 (${n} 条消息, ${ms}ms)` }] };
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                log("reply 失败:", msg);
                return { content: [{ type: "text", text: `发送失败: ${msg}` }], isError: true };
            }
        }
    ...
    log(`账号: ${creds.accountId}  白名单: ${allowSet.size ? [...allowSet].join(",") : "(未设-接受所有发送者⚠️)"}  分片上限: ${config.chunkMaxLength}`);
    log("开始轮询微信消息...");
    ...
    async function pollLoop() {
        ...
        while (true) {
            try {
                ...
                consecutiveFailures++;
                log("poll 异常:", err instanceof Error ? err.message : err, `(${consecutiveFailures}/3)`);
                if (consecutiveFailures >= 3) {
                    consecutiveFailures = 0;
                    await sleep(30_000);
                }
